...
local-start: # Build and start local infrastructure.
...
#------- CDC -------
local-start-cdc: # Starts the CDC system
	python3 -m data_ingestion.cdc

local-test-cdc: # Inserts test data into MongoDB
	python3 -m data_ingestion.test_cdc
...
local-test-github: # Send test command on local to test the lambda with a Github repository
	curl -X POST "http://localhost:9010/2015-03-31/functions/function/invocations" \
	  	-d '{"user": "Nnaemeka Ohakim", "link": "https://github.com/Nerdward/Mlops_project"}'

cloud-test-github: # Send command to the cloud lambda with a Github repository
	aws lambda invoke \
		--function-name crawler \
		--cli-binary-format raw-in-base64-out \
		--payload '{"user": "Nnaemeka Ohakim", "link": "https://github.com/Nerdward/Mlops_project"}' \
		response.json
